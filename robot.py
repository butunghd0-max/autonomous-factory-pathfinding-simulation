...
class AGV:
    ...
    def __init__(self, position: tuple, color: tuple | None = None):
        ...
        self.total_path_lengths: list[int] = []
    ...
    def assign_task(self, destination, floor, algorithm, congestion_map=None):
        ...
        if self.path:
            ...
            self.path.pop(0)  # first element is current position
            self.status = "moving"
        else:
            self.status = "blocked"
        ...
        if not self.path:
            if self.destination and self.position == self.destination:
                if self._returning_to_charge:
                    ...
                    # Chain to delivery after arriving at pickup
...
class FleetManager:
        ...
        for robot in self.robots:
            ...
            if self.task_queue:
                ...
                robot.assign_task(src, self.floor, self.algorithm,
                                  self._congestion_map)
                # Claim final destination early so others don't target it
                claimed.add(dst)
                continue
    ...
    def step_all(self, moving_obs_positions: set | None = None):
        ...
        # Seed with ALL positions so a stationary robot still blocks others
        ...
        for robot in sorted(self.robots, key=lambda r: r.id):
            # Remove self so this robot doesn't block its own movement
